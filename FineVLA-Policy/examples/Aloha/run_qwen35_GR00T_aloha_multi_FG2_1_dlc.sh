...
echo "RUNNING: QwenGR00T + Qwen3.5-4B on Aloha Multi-Dataset (7 groups, 595 tasks)"
...
config_yaml="${PROJECT_ROOT}/starVLA/config/training/starvla_aloha_multi_GR00T.yaml"
data_root=/mnt/cpfs_m6_29eu38p1/data/shared/Group-m6/tongzai.hxt
data_mix=aloha_multi_FG2_1
run_root_dir="${PROJECT_ROOT}/results/Checkpoints"
run_id=Aloha_FG2_1
pretrained_checkpoint="${PROJECT_ROOT}/results/Checkpoints/Aloha_FG2_1/checkpoints/steps_30000_pytorch_model.pt"
###########################################################################################
...
if [ -f "${wandb_id_file}" ]; then
  ...
  echo "wandb run id file not found at ${wandb_id_file}; training code will create a new run id."
fi
...
accelerate launch \
  ...
  --run_id ${run_id} \
  --wandb_project qwen35_GR00T_aloha_multi \
