...
Features:
    - Collects and visualizes evaluation success rates for checkpoints at different steps across multiple tasks.
    - Supports multiple evaluations of the same step and task, automatically averaging the results.
    - Only includes log files containing 'Average success' in the statistics.
Usage:
    python visual_eval_results.py [log_dir]
    # log_dir is the log directory path (e.g., .../output_eval), optional, defaults to the built-in path.
Output:
    - eval_results.png  visualization plot
    - eval_results.csv  success rate table for each step, task, and average
...
def visualize_ckpt_results(log_dir=None):
    ...
    # Statistics structure: step -> task -> [success_rate, ...]
...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default=None, help='Log directory path (output_eval)')
