...
class TrainerUtils:
    ...
    def _reset_dataloader(dataloader, epoch_counter):
        ...
        # 3. reset mixture dataset self.epoch key for sampling
        if hasattr(dataloader.dataset, "set_epoch") and callable(getattr(dataloader.dataset, "set_epoch", None)):
            dataloader.dataset.set_epoch(epoch_counter)
            
        # 4. create new iterator
    ...
    def _get_latest_checkpoint(self, checkpoint_dir):
        ...
        # Get all files matching the naming pattern, ensure only .pt files are matched
        checkpoints = [
