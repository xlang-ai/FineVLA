...
def setup_optimizer_and_scheduler(model, cfg) -> Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler._LRScheduler]:
    ...
    # Use scheduler_total_steps if set, otherwise fall back to max_train_steps.
    # This allows decoupling the LR schedule length from the training loop length,
    # which is critical for resume: keep the original schedule shape so LR is continuous.
    scheduler_total_steps = getattr(cfg.trainer, "scheduler_total_steps", cfg.trainer.max_train_steps)
...
class VLATrainer(TrainerUtils):
    ...
    def _adjust_lr_scheduler_for_resume(self):
        ...
        if self.completed_steps > 0:
            ...
            self.vla_epoch_count = self.completed_steps // len(self.vla_train_dataloader)
    ...
    def _init_wandb(self):
        ...
        if self.accelerator.is_main_process:
            ...
            wandb_resume = None
            wandb_id = None
            if is_resume and self.completed_steps > 0:
                # try to load previous wandb run id
                if os.path.exists(wandb_id_file):
                    ...
                    logger.warning("No wandb_run_id.txt found, creating new wandb run")
    ...
    def _init_checkpointing(self):
        ...
        if self.resume_from_checkpoint:
            ...
            self.model = self.load_pretrained_backbones(self.model, self.resume_from_checkpoint, reload_modules=reload_modules)
        
        if is_resume and self.resume_from_checkpoint:
            try:
                self.completed_steps = int(re.search(r"steps_(\d+)_pytorch_model\.pt", self.resume_from_checkpoint).group(1))
            except AttributeError:
                logger.warning(f"Could not parse steps from pretrained checkpoint: {self.resume_from_checkpoint}")
                self.completed_steps = 0
            logger.info(f"Loaded pretrained checkpoint: {self.resume_from_checkpoint}, steps: {self.completed_steps}")
        else:
    ...
    def eval_action_model(self, step_metrics: dict = None) -> float:
        ...
        def collate_fn_extend_dim(batch, max_dim=32):
            ...
            return batch
        examples = collate_fn_extend_dim(examples, max_dim=self.config.framework.action_model.action_dim) # TODO make sure action dim is extended to cross_attention_dim, think about where to put this function
