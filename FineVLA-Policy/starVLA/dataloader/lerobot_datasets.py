...
from starVLA.dataloader.gr00t_lerobot.mixtures_lerobot_data_v21_merged import EXTRA_DATASET_NAMED_MIXTURES
...
ALL_DATASET_NAMED_MIXTURES = dict(DATASET_NAMED_MIXTURES)
for mix_name, mix_spec in EXTRA_DATASET_NAMED_MIXTURES.items():
    if mix_name in ALL_DATASET_NAMED_MIXTURES:
        raise ValueError(f"Duplicate dataset mixture name: {mix_name}")
    ALL_DATASET_NAMED_MIXTURES[mix_name] = mix_spec
...
def make_LeRobotSingleDataset(
    ...
    # Detect _fg suffix for FineGrainedInstruction loading
    use_fg = robot_type.endswith("_fg")
    actual_robot_type = robot_type[:-3] if use_fg else robot_type

    data_config = ROBOT_TYPE_CONFIG_MAP[actual_robot_type]
    modality_config = data_config.modality_config()
    transforms = data_config.transform()
    dataset_path = data_root_dir / data_name
    if actual_robot_type not in ROBOT_TYPE_TO_EMBODIMENT_TAG:
        print(f"Warning: Robot type {actual_robot_type} not found in ROBOT_TYPE_TO_EMBODIMENT_TAG, using {EmbodimentTag.NEW_EMBODIMENT} as default")
        embodiment_tag = EmbodimentTag.NEW_EMBODIMENT
    else:
        embodiment_tag = ROBOT_TYPE_TO_EMBODIMENT_TAG[actual_robot_type]
    ...
    return LeRobotSingleDataset(
        ...
        data_cfg=data_cfg,
        use_fine_grained_instruction=use_fg,
    )
...
def get_vla_dataset(
    ...
    **kwargs: dict,
    ...
    mixture_spec = ALL_DATASET_NAMED_MIXTURES[data_mix]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_yaml",
        type=str,
        default="./examples/Robocasa365/train_files/starvla_train_robocasa365.yaml",
    ...
    for batch in tqdm(train_dataloader, desc="Processing Batches"):
        ...
        count += 1
        pass
    # 15996323 samples in total for xrobotv1 mixture
