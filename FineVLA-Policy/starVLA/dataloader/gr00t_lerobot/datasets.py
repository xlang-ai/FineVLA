...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        data_cfg = None,
        use_fine_grained_instruction: bool = False,
        **kwargs,
    ):
        """
        ...
        # FineGrainedInstruction support
        self.use_fine_grained_instruction = use_fine_grained_instruction
        if self.use_fine_grained_instruction:
            self._fg_instructions = self._load_fine_grained_instructions()
        # self._episodes = self._get_episode_info() # TODO why we need this func
        self.curr_traj_data = None
        self.curr_traj_id = None

        self._trajectory_ids, self._trajectory_lengths = self._get_trajectories()
        # FG mode: only keep episodes that have FG annotations
        if self.use_fine_grained_instruction and hasattr(self, '_fg_instructions') and self._fg_instructions:
            fg_ids = set(self._fg_instructions.keys())
            mask = np.array([tid in fg_ids for tid in self._trajectory_ids])
            n_before = len(self._trajectory_ids)
            self._trajectory_ids = self._trajectory_ids[mask]
            self._trajectory_lengths = self._trajectory_lengths[mask]
            print(f"FG episode filter: {n_before} -> {len(self._trajectory_ids)} episodes (kept {len(self._trajectory_ids)} with FG annotations)")
    ...
    def _get_all_steps_single_process(self) -> list[tuple[int, int]]:
        ...
        for trajectory_id, trajectory_length in tqdm(zip(self.trajectory_ids, self.trajectory_lengths), total=len(self.trajectory_ids), desc="Getting All Step"):
            try:
                ...
                # NOTE:
                # Some datasets may have metadata `episode.length` inconsistent with parquet rows.
                # Always trust the actual loaded trajectory length to avoid out-of-bounds indexing.
            ...
            for base_index in range(actual_length):
    ...
    def _load_fine_grained_instructions(self) -> dict:
        """Load FineGrainedInstruction.jsonl for this dataset."""
        fg_path = self.dataset_path / "meta" / "FineGrainedInstruction.jsonl"
        if not fg_path.exists():
            print(f"Warning: FineGrainedInstruction.jsonl not found for {self.dataset_name}, will fallback to normal instructions")
            return {}
        fg_dict = {}
        with open(fg_path, "r") as f:
            for line in f:
                entry = json.loads(line)
                fg_dict[entry["episode_id"]] = entry["FineGrainedInstruction"]
        print(f"Loaded {len(fg_dict)} FineGrainedInstructions for {self.dataset_name}")
        return fg_dict
    ...
    def get_video(
        ...
        # Always clamp by real loaded trajectory length to avoid metadata mismatch issues.
        max_length = len(self.curr_traj_data)
    ...
    def get_state_or_action(
        ...
        # Get the step indices
        step_indices = self.delta_indices[key] + base_index
        # Always use real loaded trajectory length to avoid metadata mismatch issues.
        assert self.curr_traj_data is not None, f"No data found for {trajectory_id=}"
        max_length = len(self.curr_traj_data)
    ...
    def get_language(
        ...
        """
        # FineGrainedInstruction override: concatenate all FG strings into one instruction
        if self.use_fine_grained_instruction and hasattr(self, '_fg_instructions'):
            if trajectory_id in self._fg_instructions:
                fg_list = self._fg_instructions[trajectory_id]
                instruction = " ".join(fg_list)
                step_indices = self.delta_indices[key] + base_index
                return [instruction] * len(step_indices)

        assert self.curr_traj_data is not None, f"No data found for {trajectory_id=}"
        # Get the step indices
        step_indices = self.delta_indices[key] + base_index
        # Get the maximum length from real loaded trajectory
        max_length = len(self.curr_traj_data)
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.seed = seed
        self.mode = mode
        # rank_env = os.environ.get("LOCAL_RANK", os.environ.get("RANK", "0"))
        self.rank = 2
        ...
        # Per-dataset step order for sequential sampling
    ...
    def sample_step(self, index: int) -> tuple[LeRobotSingleDataset, int, int]:
        ...
        # Sample validated step directly from precomputed all_steps.
        # This avoids relying on potentially stale/inaccurate metadata trajectory_lengths.
        if len(dataset.all_steps) == 0:
            raise ValueError(f"Dataset {dataset.dataset_name} has no available steps")
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        if self._pass_counter % 1000 == 0:
            #  force garbage collection every 100 samples
            import gc
            gc.collect()
            print(f"Pass counter: {self._pass_counter}, current index: {index}")
    ...
    @staticmethod
    def merge_metadata(
        metadatas: list[DatasetMetadata], # for the same embodiment
        dataset_sampling_weights: list[float],
        ...
        for modality, configs in modality_configs.items():
            if len(configs) > 1 and modality == "video":
                # Video configs may differ in resolution/fps across datasets (images
                # are resized to a fixed resolution during training, so this is safe).
                print(f"Warning: {len(configs)} different video configs within same tag, using first.")
            else:
                assert (
                    len(configs) == 1
    ...
    def update_metadata(self, metadata_config: dict, cached_statistics_path: Path | str | None = None) -> None:
        ...
        each_embodiment_wights = {}
        for dataset in self.datasets:
            if dataset.tag not in all_metadatas:
                all_metadatas[dataset.tag] = []
                each_embodiment_wights[dataset.tag] = []
            all_metadatas[dataset.tag].append(dataset.metadata)
            each_embodiment_wights[dataset.tag].append(self.dataset_sampling_weights[self.datasets.index(dataset)])
        
        for tag, metadatas in all_metadatas.items():
            self.merged_metadata[tag] = self.merge_metadata(
                metadatas=metadatas,
                dataset_sampling_weights=each_embodiment_wights[tag],
