...
from .default_configs.gr00t_n1d6 import Gr00tN1d6Config
from .flow_matching_head.dit import AlternateVLDiT, DiT
from .flow_matching_head.embodiment_conditioned_mlp import (
    CategorySpecificMLP,
...
class Gr00tN1d6ActionHead(nn.Module):
    ...
    def __init__(self, config: Gr00tN1d6Config, **kwargs):
        ...
        self.input_embedding_dim = config.diffusion_model_cfg["num_attention_heads"] * config.diffusion_model_cfg["attention_head_dim"] 
        ...
        self.state_encoder = CategorySpecificMLP(
            ...
            output_dim=self.input_embedding_dim, # should equal to q_dim?
        )
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        # Embed state (allow None).
        state_input = getattr(action_input, "state", None)
        state_features = (
            self.state_encoder(state_input, embodiment_id) if state_input is not None else None
        )

        # Dropout state features.
        if state_features is not None and self.state_dropout_prob > 0:
            do_dropout = (
        ...
        if (
            ...
            and self.training
            and self.state_additive_noise_scale > 0
        ):
            print(
...
def get_action_model(config=None):
    ...
    if not isinstance(config.framework.action_model, Gr00tN1d6Config):
        action_model_cfg = Gr00tN1d6Config()
        for key, value in config.framework.action_model.__dict__.items():
            setattr(action_model_cfg, key, value)

    action_model_cfg.backbone_embedding_dim = config.framework.qwenvl.vl_hidden_dim
    action_model_cfg.max_state_dim = config.framework.action_model.state_dim
    action_model_cfg.max_action_dim = config.framework.action_model.action_dim
    action_model_cfg.max_num_embodiments = getattr(
        config.framework.action_model, "n_action_heads", 1
    )
    return Gr00tN1d6ActionHead(
        config=action_model_cfg,
        full_config=config
    )
