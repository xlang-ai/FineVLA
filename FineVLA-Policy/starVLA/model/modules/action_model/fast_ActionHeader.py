...
class Fast_Action_Tokenizer(nn.Module):
    ...
    playground/Pretrained_models/fast --> https://huggingface.co/physical-intelligence/fast
    """
    ...
    def encoder_action2fastoken(self, raw_actions):
        ...
        return batch_fast_tokens # List[int]
    ...
    def encoder_action2vlmtoken(self, raw_actions):
        # x: (batch_size, chunck, dim)
        batch_fast_tokens = self.encoder_action2fastoken(raw_actions)  # List[str]

        # Map fast tokens to VLM action token format
        batch_vlm_tokens = [self.map_fast_token_to_vlm_action(fast_token_str) 
            for fast_token_str in batch_fast_tokens]
