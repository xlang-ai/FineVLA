...
class _QWen3_5_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        model = Qwen3_5ForConditionalGeneration.from_pretrained(
            model_id,
            attn_implementation="flash_attention_2", # is this the best choice for 3.5? we can try different attn implementation for better speed / memory tradeoff
    ...
    def build_qwenvl_inputs(self, images, instructions, solutions=None, **kwargs):
        ...
        # When max_seq_length is set, pad short sequences to a fixed shape to avoid per-shape
        # kernel recompilation cost when sample lengths vary widely (e.g. FineGrainedInstruction).
        # Long sequences that exceed max_seq_length are NOT truncated — they use their actual length.
        qwenvl_cfg = self.config.framework.get("qwenvl", {})
        max_seq_length = qwenvl_cfg.get("max_seq_length", None)

        if max_seq_length is not None:
            # First tokenize with dynamic padding to get actual lengths
            ...
            # If the actual length is shorter than max_seq_length, re-pad to fixed length
    ...
    def get_image_input_mask(self, input_ids):
        """
        Get image input mask from input_ids.
        Image tokens are marked as 1, others as 0.
        """
        image_mask = ((input_ids == IMAGE_TOKEN_INDEX) | (input_ids == VIDEO_TOKEN_INDEX)).long()
        return image_mask
