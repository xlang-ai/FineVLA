...
"franka": {"action_dim": 7, "NUM_ACTIONS_CHUNK": 16, "robo_info": "single arm, delta eef"}, # LIBERO
"oxe_bridge": {"action_dim": 7, "NUM_ACTIONS_CHUNK": 16, "robo_info": "single arm, delta eef"}, # OXE-Bridge
"oxe_rt1": {"action_dim": 7, "NUM_ACTIONS_CHUNK": 16, "robo_info": "single arm, delta eef"}, # OXE-RT1
"gr1": {"action_dim": 29, "NUM_ACTIONS_CHUNK": 16, "robo_info": "dual-arm dexterous hands, Joint"}, # Robocasa-GR1
"robotwin": {"action_dim": 14, "NUM_ACTIONS_CHUNK": 16, "robo_info": "arm gripper, Joint"}, # Robotwin
}
...
@FRAMEWORK_REGISTRY.register("Qwen_xGR00T_exp3")
class Qwen_xGR00T_exp3(baseframework):
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Steps:
          0. Padding action to max dim
          1. Encode with QwenVL (hidden states retained)
          2. Compute action loss with Flow-matching head
        Returns:
            dict:
                action_loss (torch.Tensor): Scalar action loss.
        """
...
if __name__ == "__main__":
    ...
    model: Qwen_xGR00T_exp3 = Qwen_xGR00T_exp3(cfg)
    print(model)
    ...
    # Create a sample
    sample = {
        "action": np.random.uniform(-1, 1, size=(16, 6)).astype(np.float16), # action_chunk, action_dim
