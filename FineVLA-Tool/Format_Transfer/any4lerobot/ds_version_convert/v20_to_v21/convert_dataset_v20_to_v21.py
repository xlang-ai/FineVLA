...
def convert_dataset(
    ...
    # Ensure info.json is v2.0 before workers load the dataset.
    # Handles re-runs where a previous conversion already bumped to v2.1:
    # without this, workers would see v2.1 and try to load episodes_stats.jsonl
    # which we are about to delete.
    if dataset.meta.info.get("codebase_version") != V20:
        dataset.meta.info["codebase_version"] = V20
        write_info(dataset.meta.info, dataset.root)

    convert_stats(dataset, num_workers=num_workers, backend=backend)
    if skip_aggregate_check:
        print("Skipping aggregate stats check (--skip-aggregate-check)")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Skip aggregate stats consistency check. Useful for subset/test datasets "
        "whose stats.json was inherited from the full dataset.",
    )
    parser.add_argument(
        "--backend",
        type=str,
        default="process",
        choices=["process", "ray"],
        help="Parallelism backend: 'process' (ProcessPoolExecutor with initializer) "
        "or 'ray' (Ray Actors with dynamic work-stealing). Defaults to 'process'.",
    )
